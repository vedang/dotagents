...
Bash is for read-only commands only: `git diff`, `git log`, `git show`, `jj diff`, `jj log`. Assume tool permissions are not perfectly enforceable; keep all bash usage strictly read-only.
