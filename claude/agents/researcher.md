...
2. **Search** - Use the `WebSearch` tool for each facet
3. **Evaluate** - Read the answers. Identify what's well-covered, what has gaps, what's noise
4. **Deep dive** - For the 2-3 most promising source URLs, use `WebFetch` to get full page content
...
- If a progress file is specified (e.g. `progress.md` in a task directory), update it as you complete each search phase
